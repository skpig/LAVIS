...
def main(model_name='facebook/opt-6.7b', dataset_name='aokvqa', split='val', bsz=2, device=0, stage=None, num_parallel_workers=1):
    ...
    if stage == 'prompt_generation':
        ...
        torch.save(rtn, f"{dataset_name}_{split}.pt")
    elif stage == 'answer_generation':
        samples = torch.load(f"{dataset_name}_{split}.pt")
...
Your should give your answer in json format. No preamble and no additional output, get right to the answer json. The requirements are as follows:
{
"QueryID": # the id of current query, for example "1" means the first query.
"TurnID": # the number of rounds of the current query.
"Reason": # your reasoning process to answer the question step-by-step.
"Action": # following the "Reason" content, must and can only choose one action at a time from the following two options:
    {
...
Do not output anything else outside the json format.
"""
        },
        {
            'role': 'user',
    ...
    while True:
        if first_turn:
            batch_cur_caption = prompt_generation([case], torch.device('cuda:0'), caption_model, vis_processors, txt_processors, num_captions=15)
            first_turn = False
        else:
            batch_cur_caption = prompt_generation([case], torch.device('cuda:0'), caption_model, vis_processors, txt_processors, num_captions=15)
        torch.cuda.empty_cache()
    ...
    "Caption" : {json.dumps(cur_caption.split("."))},
        ...
        try:
            ...
            if 'Answer' in cur_responese_dict['Action']:
                ...
                case['question'] = cur_responese_dict['Action']['Search']
                turns += 1
                if turns > 5:
                    return case['question'], history, "question"
            ...
            try:
                ...
                case['question'] = question.group(1).strip().strip(':').strip('"')
            except Exception:
    ...
    try:
        with tqdm(total = len(val_dataset)) as pbar:
            ...
            for i, item in enumerate(val_dataset):
                ...
                single['pred'], single['history'], single['pred_type'] = run(deepcopy(item))
